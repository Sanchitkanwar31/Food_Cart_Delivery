...
function Edit() {
  ...
  const handleSubmit = async(event) => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/api/edit/${foodIt._id}`, {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        console.log('Food Item item edited successfully:', data);
        navigate('/'); 
      } else {
        console.error('Error editing Foo item:', data.error);
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (state) {
      const { foodIt } = state;
      setForm({
        CategoryName: foodIt.CategoryName,
        name: foodIt.name,
        img: foodIt.img,
        options: {
          half: foodIt.options[0].half,
          full: foodIt.options[0].full
        },
        description: foodIt.description,
        type: foodIt.type
      });
    }
  }, []);
      ...
      <form onSubmit={handleSubmit} className="tw-space-y-4">
        <div className="tw-form-group">
          ...
          <input disabled 
            ...
            className="tw-block tw-w-full tw-p-2 tw-border tw-border-gray-300 tw-rounded-md tw-focus:tw-ring tw-focus:tw-ring-blue-500 tw-focus:tw-border-blue-500 tw-bg-slate-800 tw-text-white tw-cursor-not-allowed"
          />
        </div>
        <div className="tw-form-group">
